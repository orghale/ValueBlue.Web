...
namespace ValueBlue.Web.Common
{
    public class ErrorHanler
    {
        ...
        public ErrorHanler(string error)
        {
            SetValue(error);
        }

        async void SetValue(string error)
        {
            string errMsg;
            try
            {
                string errCode = await Helper.CleanString(error, "Code :", ", Message :");

                if (string.IsNullOrWhiteSpace(errCode))
                {
                    errMsg = error.Contains(err[0]) || error.Contains(err[1]) ?
                        ConstMessage.TIME_OUT_MONGO_ERROR : ConstMessage.UNKNOWN_MONGO_ERROR;
                }
                else
                {
                    int errValue = int.Parse(errCode.Trim());
                    errMsg = errValue is -1 ? error : Enum.GetName(typeof(MongoErrorCode), errValue);
                }
            }
            catch (Exception e)
            {
            ...
            Message = errMsg.Replace("_", " ").ToLower();
        }
